	...
	while(scanf("%d",&n)!=EOF && n){
		...
		for(int i=0; i<5; ++i){
			for(int j=0; j<n; ++j){
				...
				addEdge(now,now+add,1,cost);
				if(i) addEdge(now+add,now-n,1,0);
				if(i<4) addEdge(now+add,now+n,1,0);
				if(j) addEdge(now+add,now-1,1,0);
				if(j<n-1) addEdge(now+add,now+1,1,0);
				else if(j==n-1) addEdge(now+add,T,1,0);
			}
			getchar();
		}
		printf("%d\n", mincost(S,T));
